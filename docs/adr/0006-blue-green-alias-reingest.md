...
> 修订注（2026-09-17）：上句"语料上千再启用"是决策时点的粗略表述，**阈值以 OPS §5 债务闹钟表为准**
> （`chunks > 2000` **或** live 全量重灌 `> 10min`，两者都可从 `/metrics` 的 `reingest_last` 与产物测得）。
> 两处口径曾各写一个数（1000 vs 2000），本次收敛到可测的那一组，避免"同一个触发线两个值"。
