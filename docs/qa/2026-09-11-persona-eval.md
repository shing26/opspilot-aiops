...
**上游状态（用户侧待办）**：DashScope qwen-plus 充值；09-13 04:2x 终报前最小探针复测**仍 400 Arrearage**（充值未到账即无法实弹，非代码问题）。04:5x 追加分型诊断：`GET /models` **200**（key 有效、可鉴权，249 模型可见）而 qwen-plus/qwen-turbo/qwen-flash 全 400 Arrearage → **主账号计费状态未转正**（非模型开通、非 key 归属问题）；请到费用中心核对"可用额度"是否为正（充值到账亦有分钟级延迟）。到账后跑 `cd offline && ../offline/.venv/Scripts/python.exe qa_gen_quality_probes.py` 回填 V2/V3/V5/V7，并重跑 A2 全量与 A3。
