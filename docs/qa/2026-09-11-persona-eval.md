...
> 2026-09-12 跟进：metrics 观测缺口由 **Ops Console**（ADR-0009）部分吸收（state 快照+事件流+能力边界块）；
> LobeChat 壳已撤除（ADR-0007 修订），P2-10 的"过期无引导/回放节奏"在自建面板中由 401/403 三态文案与
> 只读事件流替代解决；生成质量包仍另计划。→ **已于 2026-09-13 执行完结，见 §8**。
>
> **2026-09-17 收口（本行"遗留"到此为止，逐项给出归属）**：生成质量包 **已完结**（§8）；P2-10 所指 UI **已不存在**（LobeChat 撤除，面板替代）故**关闭**；metrics 观测缺口**已入 OPS §5 债务表并带触发线**（面板已吸收依赖灯/降级/配额/在途组，余下 5xx 计数、延迟分位、别名指向、H2 健康、最近成功备份时间五项待触发）；P2-6 拒答 TTFT、query 软上限、引用样式漂移三项同样入表带触发线。**本文件的"遗留（另计划）"不再有未登记的自由项。**
...
| V2 verbatim 5 变体（live） | **PASS（5/5，09-13 13:3x 充值后实弹补跑）**：无 >80 字连续漏出，模型守规则 5 总结口径；交叉核对行如实记"护栏未触发→不适用"——出口掩码本轮未实弹（模型自觉即可合规），硬层兜底的实弹证据=mock 单测 10 用例（80/81 边界、跨句拼接、carry 重置、安全冲刷）+ 编排集成锁（stub LLM 逐字倒出→出口/缓存/审计/SF 全掩码）。此前经历：04:2x run1 因上游欠费阻断（HTTP 400 Arrearage，环境项） |
| V3 /v1 面同探针 | **PASS（13:3x）**：/v1 stream 面同 query 实调，无逐字漏出（靶文本经 /search 同 query 免 LLM 取得）——单设卡在 runPipeline 漏斗的双协议面一致性端到端补证（前版集成锁仅 sse via） |
| V4 防断言语态 | **PASS ×2 轮**：①欠费前 run1（04:2x）实测 colloquial/formal 2 变体 ×3 连跑 3/3 假设语态 ∧ 真生成≥1；②恢复后 13:3x 复核 2/2——判据诚实修正（引擎拒答=零锚定安全态不违语态；回放体即本 build 真实生成物），另以结构差异化弱问题逼破 L2 取得 `cache_hit=none` 现网真生成 1 次（假设词"疑似/需优先确认"在、无断言锚定，1f88478→本轮） |
| V5 L1 回放卫生 | **PASS（13:3x）**：V2 首条成功变体逐字重发 → `cache_hit=L1` ∧ 回放与首答逐字节相等 ∧ 无逐字漏出——掩码版进缓存、回放不泄原文的端到端锁 |
| V6 ZSET 混源计数（Redis 直读） | **PASS**：同指纹双发 → `storm:win:manual:<fp>` + `storm:win:alert:<fp>` 独立键，TTL 32/68s=30/60+10s 精确符合 |
| V7 长日志回归锁 | **PASS（13:3x）**：10069 字符、`cache_hit=none` 真链路（开场模板池+块乱序破 L2 命中）、非拒答、refs=3——grill Q1 回归锁正式上闸 |
| V8 文档一致性 | **PASS**："仅首条穿透"全仓 grep 零残留；CONTEXT.md 词条、ADR-0004 修订注、ADR-0010、OPS 闹钟表触发线行全部档 |
| W1 A2/A3 | **全量 PASS（13:3x 恢复后）**：A2 10/10（含 500 并发风暴收敛、跨租户矩阵、留痕计数）；A3 8/8（检索质量零回退，A3-1 的 ttft=None 崩溃即前记 LLM 阻断表象，恢复后自然消失——A3 脚本 None 脆性另案） |
| W2 demo.sh + 契约 | **READY**（宿主 jar 刷新后 11/11）+ 面板契约四层绿（含新 `verbatim_masked` 瓦片双向核对） |
| W3 探针预算 | **超支记录（终态）**：chat 实调累计 ~50 > 20——根因分段：①探针初版把模型"鹦鹉规则 2 话术"误判为拒答致两轮全量重跑；②熔断死锁修复的验证重跑；③上游欠费阻断-恢复的重跑（14+6+1）。教训入记忆；配额面零风险（5000/天） |
| W4 CI | **PASS**：本包 9 commit（4c3103a→d50be6d）逐次推 origin/main，CI 3-job 全绿（末次 run 34741878169 success）；CI 零 live 依赖，live gate 另由 §8 上方实跑行覆盖 |
...
**上游状态（已解决）**：DashScope qwen-plus 曾欠费（HTTP 400 Arrearage，分型诊断 `/models` 200 而三模型 chat 全 400=主账号计费未转正），用户于费用中心充值后 09-13 13:32 最小探针 **UPSTREAM_OK** 解除阻断；随即实跑 `qa_gen_quality_probes.py`（V2 5/5+交叉核对、V3、V5、V4、V7 全绿）与 A2 10/10、A3 8/8，上方各 live gate 已据实回填。本包无剩余环境阻断项。
